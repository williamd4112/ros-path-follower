...
ifeq ($(MODULE_BOUNCE_DETECT), y)
CFLAGS += -DBOUNCE_DETECT
SOURCES += bounce.cpp
endif
...
#CFLAGS += -DDEBUG_OBJECT_RECORD
#CFLAGS += -DDEBUG_BOUNCE
CFLAGS += -DDEBUG_FSM
#CFLAGS += -DDEBUG_FPS
...
test_bounce: bounce.o
	$(CC) $(CFLAGS) $(LDFLAGS) $(TEST_SOURCE_PATH)/$@.cpp $(BUILD_PATH)/$< $(INC_PATH) $(LIB_PATH) -o $(TEST_BUILD_PATH)/$@ $(LIBS) -std=c++11
